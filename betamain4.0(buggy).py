...
Animacije = {
    ...
    "bird" : {"bird": [idle_bird1, idle_bird2],
...
PLAYER1_POS = (60 / 1200 * W, H - 200*H/800)
PLAYER2_POS = (W - 100 / 1200 * W - PLAYER_WIDTH, H - 200*H/800)
player1 = Character(
    skin= skin1, 
    name=ime1,
...
class SkinSelector:
    ...
    def DetectClick(self):
        ...
        if self.input_rect2.collidepoint(mpos):
            ...
            self.active2 = False
SkinSelector = SkinSelector("", "", False, False, False, 0, 0, 2, ["bird", "goblin"], ["bird", "goblin"])
...
class levelSelector:
    ...
    def DrawlevelSelector(self):
        ...
        if self.num1 != self.levelnum and self.num1 <= levelstignut:
            pg.draw.rect(screen, self.levels[self.num1], levelcen1)
            textlevel1 = font2.render(self.levelname[self.num1], True, "black")
            Rect1 = textlevel1.get_rect()
            Rect1.center = (960, 1000)
            screen.blit(textlevel1, Rect1)
        else:
            levelcen1.center = (850, 600)
            screen.blit(locked, levelcen1)
    ...
    def DetectClick(self):
        ...
        if 600 <= mpos[0] <= 1320 and 50 <= mpos[1] <= 250:
            ...
            self.active1 = False
levelSelector = levelSelector( False, 0, 3, ["red", "blue", "yellow"], ["Iva", "Vledi", "Nena"])
def levelselector():
    global running, map, left_arrow_display_count, right_arrow_display_count
    running = True
    while running:
...
def cutscene():
    # blit pozadina, logo, petlje, decu, pres space to quit ect
    # sound play kraj one pesme, pa record screech
    # dodje iva i kaze ono kao gotova je petljovizija aj u krevet
    # deca krenu da se bune
    # dodje vledi i nena i kazu kao mozete da ostanete ako nas pobedite u
    # DENS BETLU
    # krene u skin selector i dalje sve normalno
    pass
...
def multiplejer():
    ...
    while running:
        ...
        # Draw players ZAMENITI SA ANIMACIJAMA KASNIJE
        #idle_animation1(player1, 0)
        #idle_animation2(player2, 0)
        ...
        if(True):    
            global brojac,perfekt1, perfekt2, gud1, gud2, bed1, bed2
            if perfekt1:
                perfect_score_animation1(player1, brojac)
            elif gud1:
                good_score_animation1(player1, brojac)
            elif bed1:
                bad_score_animation1(player1, brojac)
            else:
                idle_animation1(player1, brojac)
            if perfekt2:
                perfect_score_animation1(player2, brojac)
            elif gud2:
                good_score_animation1(player2, brojac)
            elif bed2:
                bad_score_animation1(player2, brojac)
            else:
            ...
            if(framecnt % 15 == 0):
                ...
                if(brojac >= 2):
                    brojac=  0
            perfekt1, perfekt2, gud1, gud2, bed1, bed2 = False, False, False, False , False, False
...
print(skor1, skor2)
newuser1 = player1.name
newscore1 = skor1
newuser2 = player2.name
newscore2 = skor2
game_data['level'] +=1
#dodaje score u dict
