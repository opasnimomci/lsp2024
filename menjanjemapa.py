        ...
        for event in pg.event.get():
            ...
            if event.type == pg.KEYDOWN:
                ...
                ##mape
                    ...
                    map = pg.image.load("planina.png")
                    map = pg.transform.scale(map, (W, H))
                elif event.key == pg.K_4:
                    map = pg.image.load("lidlbackground.png")
                    map = pg.transform.scale(map, (W, H))
                elif event.key == pg.K_5:
                    map = pg.image.load("blokbloklevelap.png")
                    map = pg.transform.scale(map, (W, H))
                elif event.key == pg.K_6:
                    map = pg.image.load("andergraund1.png")
                    map = pg.transform.scale(map, (W, H))
                elif event.key == pg.K_7:
                    map = pg.image.load("zima.jpg")
                    map = pg.transform.scale(map, (W, H))
                elif event.key == pg.K_8:
                    map = pg.image.load("pxArt.png")
                    map = pg.transform.scale(map, (W, H))
                elif event.key == pg.K_ESCAPE:
                    running = False

        if map == "white" or map == "pink":
            screen.fill(map)
        else:
            screen.blit(map, (0, 0))
